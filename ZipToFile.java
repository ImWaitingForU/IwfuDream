...
public class ZipToFile {
	 ...
	 * @throws Exception
	 */
	public static boolean upZipFile(String strZipFileName, String strZipUpDir) {
		try {
			ZipFile zfile = new ZipFile (strZipFileName, "GBK");
			...
			while (zList.hasMoreElements()) {
				...
				if (ze.isDirectory()) {
					File f = new File (strZipUpDir + ze.getName());
					f.mkdir();
					continue;
				}
				OutputStream os = new BufferedOutputStream (new FileOutputStream (
						getRealFileName(strZipUpDir, ze.getName())));
	...
	/**
	 * 解压缩功能. 将ZIP_FILENAME文件解压到当前目录下.
	 * 
	 * @throws Exception
	 */
	public static boolean upZipFile(String ZIP_FILENAME) {
		try {
			ZIP_FILENAME = ZIP_FILENAME.replace("\\", "/");
			if (ZIP_FILENAME.lastIndexOf('/') != -1){
				return upZipFile(ZIP_FILENAME, ZIP_FILENAME.substring(0, ZIP_FILENAME.lastIndexOf('/') + 1));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
